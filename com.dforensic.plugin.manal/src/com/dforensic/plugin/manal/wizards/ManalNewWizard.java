 ...
 * "man". If a sample multi-page editor (also available
...
public class ManalNewWizard extends Wizard implements INewWizard {
	...
	private void doFinish(String containerName,String fileName,IProgressMonitor monitor)throws CoreException {
		...
		// open apk 다이얼로그 띄우기 . UI 스레드로 구현하기.
		
		// file path : context : browse로 된 dialog띄우기.
		
		getShell().getDisplay().asyncExec(new Runnable() {
			
			@Override
			public void run() {
				apkDialog myd = new apkDialog(getShell());
				myd.create();
				myd.open();
			}
		});
	...
	private void throwCoreException(String message) throws CoreException {
		IStatus status =
			new Status(IStatus.ERROR, "MetricVisualizer", IStatus.OK, message, null);
